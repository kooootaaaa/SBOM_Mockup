...
**必要なAPI**
- `GET /api/Machine/options/{modelId}` - オプション一覧取得
- `POST /api/Machine/options/validate` - オプション組み合わせ検証

### 2.6 画面No.6: 部品変更画面 ⚠️**未実装**
...
**必要なテーブル設計**
- T_部品変更履歴テーブル（変更日、変更前品番、変更後品番、個数、理由等）

**必要なAPI**
- `GET /api/Parts/change-history/{machineId}` - 変更履歴取得
- `POST /api/Parts/change` - 部品変更登録
...
#### オプション管理API
- `GET /api/Options/{modelId}` - 機種別オプション一覧
- `POST /api/Options/validate` - オプション組み合わせ検証

#### 部品変更管理API
- `GET /api/PartsChange/{machineId}` - 変更履歴取得
- `POST /api/PartsChange` - 部品変更登録
- `PUT /api/PartsChange/{id}` - 変更内容更新
...
主要4画面（No.1-4）は高品質で実装完了済み。画面No.4にはツリー表示機能も追加実装。残り3画面（No.5-7）の実装により、完全なSBOM管理システムとして稼働可能。